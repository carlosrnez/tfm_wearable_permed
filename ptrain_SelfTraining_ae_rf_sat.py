...
def compute_sdt_thresholds(y_labeled, lambda_=10.0, gamma=0.95):
    """
    Calcula el umbral dinámico por clase según la fórmula SDT de FldtMatch.
 
    Parámetros
    ----------
    y_labeled : array-like
        Etiquetas del conjunto etiquetado actual (puede crecer con pseudo-etiquetas).
    lambda_ : float
        Parámetro de escudo (>1). Valores altos comprimen el rango de umbrales,
        protegiendo a las clases minoritarias de umbrales demasiado bajos.
    gamma : float
        Umbral base [0, 1]. La clase más fácil recibirá exactamente este valor.
 
    Retorna
    -------
    dict  {clase: umbral_float}
    """
    ...
    for cls, n_c in zip(classes, counts):
        sigma = n_max / n_c                          # dificultad: ≥1
        arg   = sigma + lambda_ - 1                  # siempre ≥ lambda_ > 1
        threshold = np.log(arg) / np.log(lambda_) * gamma  # log_λ(arg) · γ
        # Clamp de seguridad: nunca salir de [0, 1]
        thresholds[cls] = float(np.clip(threshold, 0.0, 1.0))
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        default=10.0,
        help=(
            "SDT shield parameter λ (>1). Controls how much minority classes "
            "are protected from excessively low thresholds. Default: 10."
        )
    )
    parser.add_argument(
        "--gamma",
        ...
        default=0.95,
        help=(
            "SDT base threshold γ ∈ (0,1]. The majority class will always "
            "receive exactly this threshold. Default: 0.95."
        )
    )
    # ─────────────────────────────────────────────────────────────────────────
...
for loop, (X_train, X_validation, X_test, y_train, y_validation, y_test, m_train, m_validation, m_test) in enumerate(data_iterator, start=1):    
    ...
    print(f"🟢 Iniciar Self-Training con SDT (λ={args.lambda_}, γ={args.gamma}) "
          f"(Pool inicial: {len(Z_unlabeled)} muestras)")
    ...
    while len(Z_unlabeled) > 0:
        ...
        # ── SDT: recalcular umbrales dinámicos en cada iteración ─────────────
        # Se recalculan con y_train_labeled actualizado para que el umbral
        # refleje la distribución real en este momento del self-training.
        sdt_thresholds = compute_sdt_thresholds(
            y_train_labeled,
            lambda_=args.lambda_,
            gamma=args.gamma
        )
        # Log informativo (solo iteración 1 o cuando cambia)
        if iteracion == 1:
            print("  [SDT] Umbrales iniciales por clase:")
            for cls, thr in sorted(sdt_thresholds.items()):
                print(f"        {cls}: {thr:.4f}")
        # ─────────────────────────────────────────────────────────────────────
        ...
        # ── Aplicar umbral específico de cada clase predicha (vectorizado) ───
        # Mapear el diccionario SDT a un array alineado con clf.classes_,
        # luego indexar directamente con argmax → comparación en C puro.
        class_thresholds_arr = np.array([
            sdt_thresholds.get(c, args.gamma) for c in clf.classes_
        ])
        pred_indices         = np.argmax(probas, axis=1)
        threshold_per_sample = class_thresholds_arr[pred_indices]
        confident_mask       = max_probas >= threshold_per_sample
        # ─────────────────────────────────────────────────────────────────────
 
        nuevas_muestras = np.sum(confident_mask)
        
        # Salir si ninguna muestra supera su umbral dinámico
        if nuevas_muestras == 0:
            print(f"  -> Iteración {iteracion}: Ninguna muestra superó su umbral SDT. Fin del bucle.")
            break
            
        print(f"  -> Iteración {iteracion}: Añadidas {nuevas_muestras} muestras. "
              f"Quedan {len(Z_unlabeled) - nuevas_muestras} en el pool.")
 
        # Desglose por clase de las pseudo-etiquetas aceptadas
        accepted_classes, accepted_counts = np.unique(pred_classes[confident_mask], return_counts=True)
        for cls, cnt in zip(accepted_classes, accepted_counts):
            print(f"     · {cls}: {cnt} muestras (umbral={sdt_thresholds.get(cls, args.gamma):.4f})")
...
csv_filename = f"metrics_SelfTraining_SAT_{args.sensor}.csv"
